...
class ElectrolytefMSA():
    def __init__(self,N,delta,species=2,a=np.array([1.0,1.0]),Z=np.array([-1,1]),rhob=np.array([0.1,0.1]),model='symmetrical'):
        ...
        # n = np.zeros((self.species,100),dtype=np.float32)
        # n[0] = np.linspace(-4,4,100)
        # n[1] = np.linspace(-4,4,100)
        # Gamma = np.zeros((100,100),dtype=np.float32)
        # Eta = np.zeros((100,100),dtype=np.float32)
        # for i in range(100):
        #     for j in range(100):     
        #         [Gamma[i,j],Eta[i,j]] = MSAbulkparameteres(np.power(10,np.array([n[0,i],n[0,j]])),self.a,self.Z,self.lB) #verified

        # self.Gammainterp = interpolate.interp2d(n[0],n[1],Gamma,kind='cubic')
        # self.Etainterp = interpolate.interp2d(n[0],n[1],Eta,kind='cubic')
    ...
    def auxiliary_quantities(self,rho):
        ...
        [self.Gamma,self.Eta] = MSAparameteres(self.n,self.a,self.Z,self.lB) #verified
        # for i in range(self.N):
        #     self.Gamma[i] = self.Gammainterp(np.log10(self.n[0,i]),np.log10(self.n[1,i]))
        #     self.Eta[i] = self.Etainterp(np.log10(self.n[0,i]),np.log10(self.n[1,i]))
        # # print(self.Gamma)
    ...
    def c1(self,rho,psi):
        ...
        # return self.c1nonMSA(rho)+self.c1long(psi)
