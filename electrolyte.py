...
def phicorr1D(x,b,a):
    ...
    funcs = [lambda x: (2*np.pi)*(a**3/24-a**2*b/4+a*b**2/2-b**2*np.abs(x)+b*np.abs(x)**2-np.abs(x)**3/3),0.0]
...
class Electrolyte():
    def __init__(self,N,delta,species=2,a=np.array([1.0,1.0]),Z=np.array([-1,1]),rhob=np.array([0.1,0.1])):
        ...
        for i in range(self.species):
            ...
            for j in range(i,self.species):
                ...
                nsig = int(0.5*aij/self.delta)
                x = np.linspace(-0.5*aij,0.5*aij,2*nsig)
    ...
    def mu(self,rhob):
        ...
        for i in range(self.species):
            ...
            # muu[i] = -self.lB*(self.Z[i]**2*self.Gammabulk+2*self.a[i]*self.Etabulk*self.Z[i])/(1+self.Gammabulk*self.a[i])
...
if __name__ == "__main__":
    ...
    if test2: 
        ...
        [varsol,Omegasol,Niter] = optimize_fire2(var,Omega,dOmegadnR,param,1.0e-4,0.02,True)
    ...
    if test3: 
        ...
        def Fpsi2(psii,n):
            nn[0,:nsig[0]] = 1.0e-16
            nn[1,:nsig[1]] = 1.0e-16
            nn[0,nsig[0]:] = n[0,nsig[0]:]*np.exp(-Z[0]*psii[nsig[0]:])
            nn[1,nsig[1]:] = n[1,nsig[1]:]*np.exp(-Z[1]*psii[nsig[1]:])
            Fele = ele.Flong(nn,psii)
            return (Fele+Gamma*psii[0])/L

        def dFpsidpsi2(psii,n):
            nn[0,:nsig[0]] = 1.0e-16
            nn[1,:nsig[1]] = 1.0e-16
            nn[0,nsig[0]:] = n[0,nsig[0]:]*np.exp(-Z[0]*psii[nsig[0]:])
            nn[1,nsig[1]:] = n[1,nsig[1]:]*np.exp(-Z[1]*psii[nsig[1]:])
    ...
    if test4: 
        ...
        psi0 = Gamma*4*np.pi*ele.lB
        psi[:nsig[0]] = 0.1*psi0*(1/kD+0.5*sigma[0]-x[:nsig[0]])
        psi[nsig[0]:] = 0.1*psi0*np.exp(-kD*(x[nsig[0]:]-0.5*sigma[0]))/kD
        ...
        def Fpsi2(psi,n):
            ...
            Fid = np.sum(nn*(np.log(nn)-1.0))*delta
            Fele = ele.Flong(nn,psi)
            return (Fid+Fele+Gamma*psi[0])/L
        ...
        var = np.log(n)
        [varsol,Omegasol1,Niter] = optimize_fire2(var,Omega,dOmegadnR,psi,1.0e-4,0.005,True)
